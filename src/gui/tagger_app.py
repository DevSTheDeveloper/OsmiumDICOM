...
class RegionSelector(QLabel):
    ...
    def set_label(self, label: str):
        """Set the label for the next region."""
        self.current_label = label
        
    def mousePressEvent(self, event):
        """Handle mouse press events to start drawing region."""
        self.begin = event.pos()
        self.end = self.begin
        self.is_drawing = True
        
    def mouseMoveEvent(self, event):
        """Handle mouse move events to update region size."""
        if self.is_drawing:
            self.end = event.pos()
            self.update()
            
    def mouseReleaseEvent(self, event):
        """Handle mouse release events to finalize region."""
        if self.is_drawing:
            self.is_drawing = False
...
class TaggerApp(QMainWindow):
    """Main window for the image tagging application."""
    ...
    def init_ui(self):
        ...
        self.setWindowTitle('X-Ray Image Tagger')
        ...
        self.file_type_combo.addItems(["DICOM Files (*.dcm)", "PNG Files (*.png)", "All Files (*)"])
    ...
    def browse_dataset(self):
        """Open dialog to browse for dataset directory."""
    ...
    def load_dataset(self):
        """Load or create dataset at specified path."""
    ...
    def load_image(self):
        ...
        if file_path:
            try:
                ...
                # Clear existing tags and regions
    ...
    def export_image(self):
        """Export the current image in selected format."""
    ...
    def add_tag(self):
        """Add a new tag to the current image."""
    ...
    def clear_regions(self):
        """Clear all regions from the image."""
        self.image_label.clear_regions()
